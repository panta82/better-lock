...
const { BetterLockOptions, OVERFLOW_STRATEGIES } = require('./options');
...
function BetterLock(options = undefined) {
	...
	if (!OVERFLOW_STRATEGIES[options.overflow_strategy]) {
		throw new InvalidArgumentError(
			options.name,
			'overflow_strategy',
			`one of (${Object.keys(OVERFLOW_STRATEGIES).join(',')})`,
	 ...
	 * @param {BetterLockOptions} [jobOptions] Job-specific options (mostly timeouts), to be applied on this job only
	 */
	function acquire(key, executor, callback, jobOptions = null) {
		...
		if (tools.isObject(jobOptions)) {
			jobOptions = new BetterLockOptions(Object.assign({}, options, jobOptions));
		} else {
			jobOptions = options;
		}
		...
		const queue = key !== undefined ? _keyQueues[key] || (_keyQueues[key] = []) : _noKeyQueue;

		const job = new LockJob(key, executor, callback);
		job.wait_timeout = jobOptions.wait_timeout;
		job.execution_timeout = jobOptions.execution_timeout;

		if (jobOptions.extend_stack_traces) {
		...
		if (options.queue_size !== null) {
			while (options.queue_size < queue.length) {
				...
				switch (options.overflow_strategy) {
					...
					default:
						throw new BetterLockInternalError(
							options.name,
							`Unexpected overflow strategy: ${options.overflow_strategy}`
						);
				}

				log(
					`Queue "${jobToKick.key}" has overflown, so ${jobToKick} was kicked out using the "${
						options.overflow_strategy
					}" strategy`
				);
				endJob(jobToKick, [
					new QueueOverflowError(options.name, jobToKick, options.overflow_strategy),
				]);
			}
		}
	...
	function executeJob(job) {
		...
		if (tools.isNumber(job.execution_timeout)) {
			job.execution_timeout_ptr = setTimeout(
	...
	function endJob(job, callbackArgs) {
		if (job.ended_at) {
			log(
				`${job} is trying to end, but it has already ended at ${job.ended_at.toISOString()}. Called with arguments: ${callbackArgs}`
			);
			return;
		}
		...
		if (queue.executing === job) {
			...
			if (index >= 0) {
				...
				throw new BetterLockInternalError(
					options.name,
					`${job} is neither currently executing, nor enqueued.`
				);
			}
		}
